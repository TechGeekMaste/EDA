    ...
    int layer)
{
    ...
    cout << cluster_layer_mapping.size();
/*
            //优化buffer位置，减小 skew
            SimulatedAnnealingParams params;
                  
            std::vector<ProblemData::Coordinate> best_buffer_locations = optimizeAllBufferLocations_SA(*Clusters, *LastClusters, cluster_layer_mapping, params );
            //输出所有聚类团的最优缓冲区位置
            for (size_t i = 0; i < best_buffer_locations.size(); ++i) {
                //std::cout << "Best buffer location for Cluster " << i + 1 << ": (" << best_buffer_locations[i].x << ", " 
                //    << best_buffer_locations[i].y << ")" << std::endl; 
                if (best_buffer_locations[i].x - P_data.buf_size.x >= P_data.die_area[0].x && best_buffer_locations[i].x + P_data.buf_size.x <= P_data.die_area[2].x 
                    && best_buffer_locations[i].y - P_data.buf_size.y >= P_data.die_area[0].y && best_buffer_locations[i].y + P_data.buf_size.y <= P_data.die_area[2].y ){
                    //更新聚类中心位置
                    Clusters->at(i).SetCX(best_buffer_locations[i].x); // 更新X坐标
                    Clusters->at(i).SetCY(best_buffer_locations[i].y); // 更新Y坐标
                }
                else {
                       std::cout << "Buffer location for Cluster " << i+1 << "exceeds floorpan \n";                                                                                                                       }
                      }
*/
