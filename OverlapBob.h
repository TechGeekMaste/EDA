...
struct Point
{
    static const int DIM = 2; // 维度是2
    double coords[DIM];       // 坐标数组
    int layer;                // 聚类的层数，0代表ff，1代表第一层聚类中心，值越大层次越高
    double length;            // 长度
    double width;             // 宽度

    Point(double x = 0, double y = 0, int layer = 0, double length = 0, double width = 0)
        : layer(layer), length(length), width(width)
    {
        coords[0] = x;
        coords[1] = y;
    }

    double operator[](size_t idx) const { return coords[idx]; } // 访问坐标
    double &operator[](size_t idx) { return coords[idx]; }      // 修改坐标
};
...
public:
    //
    void setCurrentBuf(const Point &buf)
    {
        current_buf = buf;
    }

    Overlap(std::vector<Point> &points_ff, std::vector<Point> &points_buf) // 创建一个Overlap类
        : points_ff(points_ff), points_buf(points_buf)                     // 初始化成员变量
    {
        points_.reserve(points_ff.size() + points_buf.size()); // 预分配空间
        points_.insert(points_.end(), points_ff.begin(), points_ff.end());
        points_.insert(points_.end(), points_buf.begin(), points_buf.end());
        kdTree_ = std::unique_ptr<kdt::KDTree<Point>>(new kdt::KDTree<Point>(points_));
    }

    const std::vector<Point> &getAdjustedPoints() const; // 获取调整后的点集
                                                         /*
                                                              const std::unordered_set<int> &getAdjustedIndices() const // 获取调整后的点集索引
                                                               {
                                                               return adjustedIndices;
                                                                                      }*/

    ProblemData::Coordinate OverlapBob(Point buf);
    double calculateCost(ProblemData::Coordinate &best_locations); // 计算候选缓冲区位置的成本(重叠总面积)

    // void resolveOverlap(int index, const std::vector<int> &neighbors, std::unordered_set<int> &adjustedIndices); // 处理重叠

    static REAL bobyqa_obj(const INTEGER n, const REAL *x, void *data);
...
private:
    ...
    std::unique_ptr<kdt::KDTree<Point>> kdTree_; // KDTree对象
                                                 // std::unordered_set<int> adjustedIndices;

    // REAL bobyqa_obj(const INTEGER n, const REAL *x, void *data);
};

// 计算候选缓冲区位置的成本(重叠总面积)
double Overlap::calculateCost(ProblemData::Coordinate &candidate_location)
{
    // std::cout << "Candidate Location: ( " << candidate_location.x << ", " << candidate_location.y << ") " << std::endl;
    // adjustedIndices.clear();    //清空
    double overlap_area = 0;
    double minDist;
    int nnIndex;
    double radius_;
    std::vector<int> neighbors;
    Point candidate_buf(candidate_location.x, candidate_location.y, 1, current_buf.length, current_buf.width);
    // 最近邻搜索
    nnIndex = kdTree_->nnSearch(candidate_buf, &minDist);
    radius_ = 0.5 * (points_ff[0].length + current_buf.length);
    if (minDist > radius_) // 若大于radius_，则不存在重叠
        return 0;

    else
    {
        // 查找该点在radius_范围内的邻居
        neighbors = kdTree_->radiusSearch(candidate_buf, radius_);

        // 除去邻居中的自己和不重叠的邻居
        neighbors.erase(std::remove_if(neighbors.begin(), neighbors.end(),
                                       [&](int neighborIdx)
                                       {
                                           bool IsTrue1 = abs(points_[neighborIdx].coords[1] - candidate_buf.coords[1]) > 0.5 * (points_[nnIndex].width + candidate_buf.width);
                                           bool IsTrue2 = CountDistance(candidate_buf.coords[0], candidate_buf.coords[1], points_[neighborIdx].coords[0], points_[neighborIdx].coords[1]) > 0.5 * (points_[neighborIdx].length + candidate_buf.length);
                                           return IsTrue1 || IsTrue2; // 返回 true 则会被移除
                                       }),
                        neighbors.end());

        if (neighbors.empty()) // 如果找到的邻居数为0
            return 0;
        ...
        else
        {
            ...
            {
                // std::cout << "neighbor " << index << ": (" << points_[neighborIdx].coords[0] << ", " << points_[neighborIdx].coords[1] << ")" << std::endl;
                // 计算重叠坐标差
                double dif_x = 0.5 * (points_[neighborIdx].length + candidate_buf.length) - abs(points_[neighborIdx].coords[0] - candidate_buf.coords[0]);
                double dif_y = 0.5 * (points_[neighborIdx].width + candidate_buf.width) - abs(points_[neighborIdx].coords[1] - candidate_buf.coords[1]);
                overlap_area += dif_x * dif_y;
            }

            std::cout << overlap_area << std::endl;
            return overlap_area; // 返回计算的重叠面积
        }
    }
}
...
REAL Overlap::bobyqa_obj(const INTEGER n, const REAL *x, void *data)
{
    Overlap *instance = static_cast<Overlap *>(data);
    ProblemData::Coordinate candidate_location{x[0], x[1]};
    return instance->calculateCost(candidate_location);
}

ProblemData::Coordinate Overlap::OverlapBob(Point buf)
{
    ProblemData::Coordinate best_location = {buf.coords[0], buf.coords[1]}; // 初始化最佳位置
    double min_cost = calculateCost(best_location);                         // 计算当前成本
    if (!min_cost)
        return best_location;

    double w = buf.width;  // buf的宽
    double l = buf.length; // buf的长

    // 为工作空间分配内存
    double *workspace = new double[10000];
    //  创建 x 数组，存储当前点的坐标
    REAL x[2] = {buf.coords[0], buf.coords[1]};

    // 创建上下界数组
    REAL xu[2] = {buf.coords[0] -  l, buf.coords[0] +  l};
    REAL xl[2] = {buf.coords[1] -  w, buf.coords[1] +  w};

    //  调用 BOBYQA 优化算法
    bobyqa(2, 5,
           bobyqa_obj, (void *)this,
           x, xl, xu,
           w * 0.07, 0.5,
           1, 100, workspace);
    ...
    std::cerr << "cost_sum: " << calculateCost(best_location) << "\n";
    return best_location;
}
...
/*---------------------------------------------------------------------------*/
int bobyqa(const INTEGER n, const INTEGER npt,
           bobyqa_objfun *objfun, void *data,
           REAL *x, const REAL *xl, const REAL *xu,
           const REAL rhobeg, const REAL rhoend,
