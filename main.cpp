...
int main(int argc, char* argv[]) {
   ...
   // clusters = AllClusters[2];
    //clusters->at(35).Print();
    
    /*
    //查找并输出缓冲区位置
    double search_radius = 100;
    vector<ProblemData::Coordinate> buffer_locations = findBufferLocation(Clusters_Layer1, Clusters_Layer2, cluster_layer2_mapping, search_radius);
    cout << "Buffer Locations:" << endl;
    for(size_t i = 0; i < buffer_locations.size(); ++i)
    {
      #pragma omp criticial
      cout << "Buffer for Locations " << i+1 << ": (" << buffer_locations[i].x << "," << buffer_locations[i].y << ")" <<endl;
    }
*/
    // 格式输出
